...
public class Neo4jLocalTxnInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        try {
            ...
            // Close the session if necessary (guarded so this code doesn't run
            // unless catch fired).
            if (null != workStarted.get() && (txn.status().equals(Transaction.Status.CLOSED)
                    || txn.status().equals(Transaction.Status.COMMITTED)
